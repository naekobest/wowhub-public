...
- **HealingMetricsService** — tracks HPS per player with the same trash / boss / total breakdown. Previously tracked overall healing only; now segments are available for deeper per-fight analysis.
...
- WCL API Key setup added to `/settings/api` with a step-by-step guide and copy buttons for the required OAuth scopes.
